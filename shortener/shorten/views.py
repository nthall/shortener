...
def generate_code():
    pop = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ' * 2 + '01234567890123456789'
...
def check_code(candidate):
    code = candidate
    try: 
        match = UrlPair.objects.get(code__exact=code)
    ...
    if request.POST['destination_url']:
      ...
      #todo: smarter protocol check.
        ...
        try:
            ...
            data = {'url': match, 'alert': "This url has already been shortened! I guess great minds link alike."}
            return render_to_response('result.html', data) 
    else:
        data = {'error_message': "Looks like you didn't enter a valid link. If at first you don't succeed..."}
...
def send_away(request, code):
    try:
        ...
        data = {'error_message': "Oops, that's not a valid shortlink. Maybe you'd like to shorten something else?"}
        return render_to_response('index.html', data, context_instance=RequestContext(request))
    except:
        raise Http404
    else:
